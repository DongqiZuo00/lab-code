...
if True:
    ...
    for j in range(3):
        ...
        figAtt.axes[axNo].plot(dat['/estimator'+vehID]['t'], dat['/estimator'+vehID]['att'+angle], color='b', label='est')
        figAtt.axes[axNo].plot(dat['/mocap_output'+vehID]['t'], dat['/mocap_output'+vehID]['att'+angle], color='r', label='mocap')
        figAtt.axes[axNo].set_ylabel(angle)
    #plot ang velocity
    for j in range(3):
        axNo = 1*3+j
        figAtt.axes[axNo].plot(dat['/estimator'+vehID]['t'], dat['/estimator'+vehID]['angvel'+'xyz'[j]],color='b', label='est')
        figAtt.axes[axNo].plot(dat['/telemetry'+vehID]['t'], dat['/telemetry'+vehID]['rateGyro'][:,j],color='g', label='gyro')
        figAtt.axes[axNo].plot(dat['/radio_command'+vehID]['t'], dat['/radio_command'+vehID]['debugvals'][:,1+j],color='r', label='cmd')
