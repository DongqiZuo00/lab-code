...
for j in range(3):
    axNo = 1*3+j
    figPos.axes[axNo].plot(dat['/estimator'+vehID]['t'], dat['/estimator'+vehID]['att'+att[j]], color='b', label='est')
    figPos.axes[axNo].plot(dat['/mocap_output'+vehID]['t'], dat['/mocap_output'+vehID]['att'+att[j]], color='r', label='mocap')
    figPos.axes[axNo].set_ylabel('Att_'+att[j])
