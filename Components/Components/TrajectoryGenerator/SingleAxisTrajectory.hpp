...
public:
  ...
  SingleAxisTrajectory(void)
    :_a(0),_b(0),_g(0),_cost(std::numeric_limits<Real>::max())
  {
    Reset();
  }

  //! Define the trajectory's initial state (position, velocity, acceleration), at time zero
  void SetInitialState(const Real pos0, const Real vel0, const Real acc0){_p0=pos0; _v0=vel0; _a0=acc0; Reset();};

  //! Define the trajectory's final position (if you don't call this, it is left free)
  void SetGoalPosition(const Real posf)    {_posGoalDefined = true; _pf = posf;};

  //! Define the trajectory's final velocity (if you don't call this, it is left free)
  void SetGoalVelocity(const Real velf)    {_velGoalDefined = true; _vf = velf;};

  //! Define the trajectory's final acceleration (if you don't call this, it is left free)
  void SetGoalAcceleration(const Real accf){_accGoalDefined = true; _af = accf;};
  ...
  void GenerateTrajectory(const Real Tf)
  {
    ...
    //Calculate the cost:
    _cost =  _g*_g + _b*_g*Tf + _b*_b*T2/Real(3.0) + _a*_g*T2/Real(3.0) + _a*_b*T3/Real(4.0) + _a*_a*T4/Real(20.0);
  }
  ...
  Real GetJerk(Real t)         const{return _g  + _b*t  + (1/Real(2.0))*_a*t*t;};

  //! Returns the acceleration at time t
  Real GetAcceleration(Real t) const{return _a0 + _g*t  + (1/Real(2.0))*_b*t*t  + (1/Real(6.0))*_a*t*t*t;};

  //! Returns the velocity at time t
  Real GetVelocity(Real t)     const{return _v0 + _a0*t + (1/Real(2.0))*_g*t*t  + (1/Real(6.0))*_b*t*t*t + (1/Real(24.0))*_a*t*t*t*t;};

  //! Returns the position at time t
  Real GetPosition(Real t)     const{return _p0 + _v0*t + (1/Real(2.0))*_a0*t*t + (1/Real(6.0))*_g*t*t*t + (1/Real(24.0))*_b*t*t*t*t + (1/Real(120.0))*_a*t*t*t*t*t;};
  ...
  Real GetParamAlpha(void) const {return _a;};
  //! Get the parameters defining the trajectory
  Real GetParamBeta (void) const {return _b;};
  //! Get the parameters defining the trajectory
  Real GetParamGamma(void) const {return _g;};
  //! Get the parameters defining the trajectory
  Real GetInitialAcceleration(void) const {return _a0;};
  //! Get the parameters defining the trajectory
  Real GetInitialVelocity(void)     const {return _v0;};
  //! Get the parameters defining the trajectory
  Real GetInitialPosition(void)     const {return _p0;};

  //! Get the trajectory cost value
  Real GetCost(void) const{return _cost;};

private:
  Real _p0, _v0, _a0;//!< The initial state (position, velocity, acceleration)
